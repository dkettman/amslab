...
resource "hyperv_vhd" "diff_win2k22" {
  path        = "F:\\Hyper-V\\${var.server_name}.vhdx"
  parent_path = "c:\\users\\Work\\git\\packer\\output-win2k22\\Virtual Hard Disks\\packer-win2k22.vhdx"
  vhd_type    = "Differencing"
}

resource "hyperv_machine_instance" "win2k22" {
  ...
  hard_disk_drives {
    ...
    controller_number   = "0"
    controller_location = "0"
    path                = hyperv_vhd.diff_win2k22.path
  }
}
